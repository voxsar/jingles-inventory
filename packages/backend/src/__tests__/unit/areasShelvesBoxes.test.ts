...
describe('GET /api/floors', () => {
	...
	it('returns list of floors', async () => {
		const floors = [
			{ id: 'floor-001', branchId: 'branch-001', name: 'Ground Floor', code: 'GF', floorNumber: 1, length: null, width: null, isActive: true, createdAt: new Date(), branch: null, _count: { inventoryRecords: 0, shelves: 0 } },
		];
...
describe('POST /api/floors', () => {
	...
	it('creates a floor as Admin', async () => {
		const newFloor = { id: 'floor-002', branchId: 'aad8f10c-3b0a-4bce-b4c0-8e5400f20001', name: 'Ground Floor', code: 'GF', floorNumber: 1, length: null, width: null, isActive: true, createdAt: new Date(), branch: null };
		(prismaMock.floor.create as ReturnType<typeof vi.fn>).mockResolvedValue(newFloor);

		const res = await request(app)
			.post('/api/floors')
			.set('Authorization', `Bearer ${makeToken('Admin')}`)
			.send({ branchId: 'aad8f10c-3b0a-4bce-b4c0-8e5400f20001', name: 'Ground Floor', code: 'GF', floorNumber: 1 });
...
describe('DELETE /api/floors/:id', () => {
	it('soft-deletes a floor', async () => {
		const floor = { id: 'aad8f10c-3b0a-4bce-b4c0-8e5400f20001', branchId: 'branch-001', name: 'Ground Floor', code: 'GF', floorNumber: 1, length: null, width: null, isActive: true, createdAt: new Date() };
