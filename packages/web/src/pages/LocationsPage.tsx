...
const defaultFloorForm = { branchId: '', name: '', code: '', notes: '', length: '', width: '', floorNumber: '' };
...
export default function LocationsPage() {
	...
	useEffect(() => { loadBranches(); loadFloors(); }, []);
	...
	const handleCreateFloor = async (e: React.FormEvent) => {
		...
		try {
			await floorsApi.create({
				...
				length: floorForm.length ? parseFloat(floorForm.length) : undefined,
				width: floorForm.width ? parseFloat(floorForm.width) : undefined,
			});
	...
	const handleSaveEditFloor = async () => {
		...
		try {
			await floorsApi.update(editingFloor.id, {
				...
				floorNumber: editFloorForm.floorNumber ? parseInt(editFloorForm.floorNumber) : undefined,
				notes: editFloorForm.notes || undefined,
				length: editFloorForm.length ? parseFloat(editFloorForm.length) : undefined,
				width: editFloorForm.width ? parseFloat(editFloorForm.width) : undefined,
			});
		...
		{ key: 'code', header: 'Code', render: (r: any) => <span className="font-mono text-xs bg-gray-100 px-1.5 py-0.5 rounded">{r.code}</span>, sortable: true },
		{
			...
			render: (r: any) => (
				<div className="text-xs text-gray-600">
					{r.length && r.width ? `${r.length}m × ${r.width}m` : r.length ? `${r.length}m L` : r.width ? `${r.width}m W` : '—'}
				</div>
			),
		},
		{
						...
						<form onSubmit={handleCreateFloor}>
							<div className="modal-body form-stack">
								...
								<div className="form-group">
									...
									<input className="input-field" type="number" required placeholder="e.g. 0, 1, 2" min="1" value={floorForm.floorNumber} onChange={(e) => setFloorForm(f => ({ ...f, floorNumber: e.target.value }))} />
								</div>
								<div className="form-group">
								...
								<div className="form-grid-2">
									<div className="form-group">
										...
										<input className="input-field" type="number" placeholder="e.g. 50" step="0.01" min="0" value={floorForm.length} onChange={(e) => setFloorForm(f => ({ ...f, length: e.target.value }))} />
									</div>
									<div className="form-group">
										<label className="form-label">Width (meters)</label>
										<input className="input-field" type="number" placeholder="e.g. 30" step="0.01" min="0" value={floorForm.width} onChange={(e) => setFloorForm(f => ({ ...f, width: e.target.value }))} />
									</div>
								</div>
							</div>
							<div className="modal-footer">
				...
				<div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setEditingFloor(null)}>
					<div className="modal-panel-md">
						...
						<div className="modal-body form-stack">
							...
							<div className="form-group">
								<label className="form-label">Floor Number *</label>
								<input className="input-field" type="number" required placeholder="e.g. 0, 1, 2" min="1" value={editFloorForm.floorNumber} onChange={(e) => setEditFloorForm(f => ({ ...f, floorNumber: e.target.value }))} />
							</div>
							<div className="form-group">
								<label className="form-label">Notes</label>
							...
							<div className="form-grid-2">
								<div className="form-group">
									<label className="form-label">Length (meters)</label>
									<input className="input-field" type="number" placeholder="e.g. 50" step="0.01" min="0" value={editFloorForm.length} onChange={(e) => setEditFloorForm(f => ({ ...f, length: e.target.value }))} />
								</div>
								<div className="form-group">
									<label className="form-label">Width (meters)</label>
									<input className="input-field" type="number" placeholder="e.g. 30" step="0.01" min="0" value={editFloorForm.width} onChange={(e) => setEditFloorForm(f => ({ ...f, width: e.target.value }))} />
								</div>
							</div>
						</div>
						<div className="modal-footer">
