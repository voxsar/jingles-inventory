...
export default function BranchesPage() {
	...
	const openEditFloor = (f: any) => { setEditingFloor(f); setFloorForm({ name: f.name, code: f.code, notes: f.notes ?? '', length: f.length ?? '', width: f.width ?? '', floorNumber: f.floorNumber ?? '' }); setShowFloorModal(true); };
	const handleSaveFloor = async (e: React.FormEvent) => {
		...
		try {
			...
			if (editingFloor) await floorsApi.update(editingFloor.id, { ...floorForm, ...dims });
			else await floorsApi.create({ ...floorForm, ...dims, branchId: selectedBranch.id, code: floorForm.code.toUpperCase() });
						...
						<form onSubmit={handleSaveFloor}>
							<div className="modal-body form-stack">
								...
								<div className="form-group">
									...
									<input className="input-field" type="number" required min="1" placeholder="e.g. 1" value={floorForm.floorNumber} onChange={e => setFloorForm(f => ({ ...f, floorNumber: e.target.value }))} />
								</div>
								<div className="form-group">
								...
								<div className="form-grid-2">
									<div className="form-group">
										...
										<input className="input-field" type="number" step="0.01" min="0" placeholder="e.g. 50" value={floorForm.length} onChange={e => setFloorForm(f => ({ ...f, length: e.target.value }))} />
									</div>
									<div className="form-group">
										<label className="form-label">Width (m)</label>
										<input className="input-field" type="number" step="0.01" min="0" placeholder="e.g. 30" value={floorForm.width} onChange={e => setFloorForm(f => ({ ...f, width: e.target.value }))} />
									</div>
								</div>
							</div>
							<div className="modal-footer">
